...
#define CC_AF   AF0
    ...
    ISpi.Setup(boMSB, cpolIdleLow, cphaFirstEdge, sclkDiv4); // 20/4 == 5
    ISpi.Enable();
    // ==== Init CC ====
    if(Reset() != retvOk) {
        ISpi.Disable();
...
uint8_t cc1101_t::Receive(uint32_t Timeout_ms, void *Ptr, uint8_t Len, int8_t *PRssi) {
    return Receive_st(MS2ST(Timeout_ms), Ptr, Len, PRssi);
}

uint8_t cc1101_t::Receive_st(systime_t Timeout_st, void *Ptr, uint8_t Len, int8_t *PRssi) {
    FlushRxFIFO();
    chSysLock();
    EnterRX();
...
//    PrintfI("S1: %X\r", IState);
    if((IState & 0x70) != 0x10) { // Not in RX
        EnterRX();
        chThdSleepS(US2ST(117)); // 100...150 uS for 250kBaud
    }
    EnterTX();
    // Where we are?
    chThdSleepS(US2ST(108));
    GetStatus();
...
uint8_t cc1101_t::RxIfNotYet_st(systime_t RxTimeout_st, void *PtrRx, uint8_t Len,  int8_t *PRssi) {
    // Enter RX if not yet
    chSysLock();
    GetStatus();
