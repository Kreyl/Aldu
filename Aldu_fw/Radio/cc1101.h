...
public:
    ...
    uint8_t Receive_st(systime_t Timeout_st, void *Ptr, uint8_t Len,  int8_t *PRssi=nullptr);
    void ReceiveAsync(ftVoidVoid Callback);

    uint8_t RxCcaTx_st(void *PtrTx, uint8_t Len,  int8_t *PRssi=nullptr);
    uint8_t RxIfNotYet_st(systime_t RxTimeout_st, void *Ptr, uint8_t Len,  int8_t *PRssi=nullptr);
