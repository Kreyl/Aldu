...
#define LCD_DMA_TX_MODE     STM32_DMA_CR_CHSEL(LCD_DMA_CHNL) | \
                            DMA_PRIORITY_LOW | \
...
#else
    PinSetupAlterFunc(LCD_SCLK, omPushPull, pudNone, LCD_AF, psHigh);
    PinSetupAlterFunc(LCD_SDA,  omPushPull, pudNone, LCD_AF, psHigh);
    // ==== USART init ==== clock enabled, idle low, first edge, enable last bit pulse
    LCD_UART_EN();
    LCD_UART->CR1 = USART_CR1_UE;     // Enable
    LCD_UART->BRR = Clk.APB2FreqHz / LCD_UART_SPEED;
    LCD_UART->CR2 = USART_CR2_CLKEN | USART_CR2_LBCL; // Enable clock, enable last bit clock
    LCD_UART->CR1 = USART_CR1_UE | USART_CR1_M | USART_CR1_TE;
    LCD_UART->CR3 = USART_CR3_DMAT;   // Enable DMA at transmitter
#endif
    // DMA
    dmaStreamAllocate     (LCD_DMA, IRQ_PRIO_LOW, nullptr, nullptr);
    dmaStreamSetPeripheral(LCD_DMA, &LCD_UART->DR);
